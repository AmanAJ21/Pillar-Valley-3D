...
// ═══════════════════════════════════════════════════════════════════════════════
// COLLISION DETECTION - Improved for projection alignment
// ═══════════════════════════════════════════════════════════════════════════════
export const checkBallCollision = (game) => {
  ...
  // Use fixed rotation radius to match projection system
  const ballX = currentPillar.x + Math.cos(rad) * CONFIG.ROTATION_RADIUS;
  const ballZ = currentPillar.z + Math.sin(rad) * CONFIG.ROTATION_RADIUS;

  // Check collision with all pillars (more forgiving)
  for (let i = 1; i < game.pillars.length; i++) {
    const p = game.pillars[i];
    const d = distance(ballX, ballZ, p.x, p.z);
    
    // More generous collision detection - use full pillar radius plus tolerance
    const collisionRadius = p.r * CONFIG.TOLERANCE;
