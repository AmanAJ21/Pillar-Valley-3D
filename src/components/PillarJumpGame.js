...
export default function PillarJumpGame() {
  ...
  const jump = useCallback(() => {
    ...
    const now = Date.now();
    if (jump.lastCall && now - jump.lastCall < 30) return;
    jump.lastCall = now;
  ...
  const onPillarReached = useCallback(() => {
    setGame(prevGame => {
      try {
        ...
        // Enhanced score feedback based on performance
        if (newScore % 5 === 0) {
          HapticFeedback.success(); // Every 5 points
        } else {
          HapticFeedback.light(); // Regular score
        }
  ...
  return (
    <MobileFullscreenWrapper>
      <View style={styles.container}>
        ...
        {/* Touch area for jumping - projection is always visible */}
        {game.playing && !game.paused && (
          <TouchableOpacity
            style={styles.touchArea}
            onPress={jump} // Jump when touched
            activeOpacity={0.8}
              ...
              <View style={styles.pauseContent}>
                ...
                <Text style={[styles.pauseInstructions, { color: colorToHex(currentTheme.ball) }]}>
                  {Platform.OS === 'web' ?
                    ...
                    'Touch and hold to see projection • Release to jump to target pillar • Ball shrinks over time'
                  }
                </Text>
              </View>
            </View>
